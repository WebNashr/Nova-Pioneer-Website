	...
	window.GPNestedForms = function( args ) {
		...
		self.init = function() {
			...
			if( typeof window[ 'GPNestedForms_{0}_{1}'.format( self.formId, self.fieldId ) ] !== 'undefined' ) {
				...
				gform.removeFilter( 'gform_calculation_formula', 10, 'gpnf' );
			}
			...
			if( self.entries.length <= 0 && window.gpnfNestedEntries[ self.formId ] ) {
				self.entries = window.gpnfNestedEntries[ self.formId ][ self.fieldId ];
			}
			...
			gform.addFilter( 'gform_calculation_formula', self.parseCalcs, 10, 'gpnf' );
        ...
        self.initFormScripts = function( currentPage ) {
            ...
            self.modalElem.find(':input').each(function () {
                ...
                var parentMergeTagMatches = /{Parent:(\d+(\.\d+)?)}/g.exec(value);
